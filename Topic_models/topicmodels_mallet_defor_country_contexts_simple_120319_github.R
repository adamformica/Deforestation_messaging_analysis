...
#' Create summary tables showing the names for each topic conext 
...
countries_topics_names <- left_join(countries_topics,literature_topic_names,by="Topic") %>%
  select(-Label)
...
load("Data/lit_outliers.Rdata")

countries_topics_top$UnderRepresented <- ifelse(countries_topics_top$Country %in% lit_outliers, "Yes", "No")
...
write.csv(countries_topics_top,
          "Manuscript_figures/literature_deforestation_top_contexts_simple.csv",
          row.names = FALSE)
